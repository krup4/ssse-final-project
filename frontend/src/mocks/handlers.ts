...
import { alerts, history, overview, parameterErrorTrend, parameterErrors, regions, stationSeries, stations, users, worstErrors } from "./data";
...
export const handlers = [
  http.post(endpoint("/auth/login"), async ({ request }) => {
    ...
    const roleFromEmail = body.email.split("@")[0] as UserRole;
    const user = users.find((item) => item.email === body.email) ?? users.find((item) => item.role === roleFromEmail) ?? users[1];
  ...
  http.get(endpoint("/regions"), () => HttpResponse.json(regions)),
